...
// MIDDLEWARE
app.use(cors({
  origin: 'http://localhost:*', // Untuk Flutter dev
  credentials: true
}));
...
// SESSION CONFIG
app.use(session({
  secret: 'campusbay-secret-key-2024', // Ganti dengan random string untuk production
  resave: false,
...
app.get('/auth/me', requireAuth, (req, res) => {
  db.get(
    'SELECT id, email, name, campus, major, year FROM users WHERE id = ?',
    [req.session.userId],
    (err, user) => {
      if (err || !user) {
...
// ADD PRODUCT (REQUIRE LOGIN)
app.post('/products', requireAuth, (req, res) => {
  const { title, price, category, description, condition, campus, image_url } = req.body;
...
app.get('/products/my/listings', requireAuth, (req, res) => {
  db.all(
...
app.put('/products/:id', requireAuth, (req, res) => {
  const { title, price, category, description, condition, image_url } = req.body;
...
// MARK AS SOLD
app.patch('/products/:id/sold', requireAuth, (req, res) => {
  db.get(
    'SELECT user_id FROM products WHERE id = ?',
    [req.params.id],
    (err, product) => {
...
app.listen(PORT, () => {
  ...
  console.log(`   POST /auth/register - Daftar akun baru`);
  console.log(`   POST /auth/login - Login`);
  console.log(`   GET  /products - Lihat semua produk`);
  console.log(`   POST /products - Tambah produk (login required)`);
  console.log(`\n💡 Test: http://localhost:${PORT}/seed untuk isi data dummy\n`);
});
